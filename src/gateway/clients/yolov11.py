# src/gateway/clients/yolov11.py
...
def _get_meta(raw: Any) -> Dict[str, Any]:
    """
    infer 서버 응답이
      raw["meta"] 또는 raw["raw"]["meta"] 형태로 오는 것을 모두 수용
    """
    if not isinstance(raw, dict):
        return {}
    m = raw.get("meta")
...
def _fill_missing_items(req: InferBatchRequest, results: List[InferItemResult]) -> List[InferItemResult]:
    """
    결과 누락 시 req.items 기준으로 빈 결과 채움
    """
    ...
    *,
    ...
    try:
        ...
        return ModelPredResponse(
            ...
            results=results,
            raw=raw_dict,  # ✅ meta 유지 → gateway에서 weight_dir_used 로그 가능
        )
